...
import { generateBrief as buildProbeBrief } from './brief/generator.ts';
import { id as newId, now as isoNow } from './core/util.ts';
...
Commands
  ...
  contract-test [--live --to N --yes] [--idempotency]
                                  Probe the voice platform's dispatch API
...
`);
      ...
      case 'contract-test': {
        ...
        if (live && (!flags.to || flags.yes !== 'true')) {
          return fail(
            [
        ...
        const { brief } = await buildProbeBrief(ctx.guardrails);
        const probeLead = {
          leadId: newId('probe'),
          runId: 'contract_probe',
          name: 'Contract Probe',
          phoneE164: flags.to ?? '+10000000000',
          email: null,
          consent: true,
        ...
        const result = await probeDispatchContract(
          { env: ctx.env, live, to: flags.to, checkIdempotency: flags.idempotency === 'true' },
          probeLead,
          brief,
        );
        ...
        for (const finding of result.findings) {
          process.stdout.write(
            `  ${mark[finding.status].padEnd(5)} ${finding.check.padEnd(14)} ${finding.detail}\n`,
          );
          if (finding.fix) process.stdout.write(`        ${' '.repeat(14)} fix: ${finding.fix}\n`);
        }
